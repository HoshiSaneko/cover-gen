...
def generate_image_prompt(
    ...
    negative = (negative_prompt or Config.NEGATIVE_PROMPT or "").strip()
    ...
    # 额外提示词要求（让 LLM 在生成时参考，避免与后追加的 extra 冲突）
    if extra:
        extra_req = f"""6. **额外要求**：画面风格或核心元素必须与以下关键词协调一致，在描述中自然融入：{extra}。"""
    else:
        extra_req = ""

    # 禁止的提示词（让 LLM 在生成时避免描述这些内容，与生图 API 的负向提示一致）
    if negative:
        neg_req = f"""7. **禁止出现**：画面中不得出现或体现以下内容，提示词中也不要包含这些关键词：{negative}。"""
    else:
        neg_req = ""
    ...
    prompt_instruction = PROMPT_INSTRUCTION_TEMPLATE.format(
        ...
        extra_prompt_requirement=extra_req,
        negative_prompt_requirement=neg_req
    )
